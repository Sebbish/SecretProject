...
public class EntityManager {
	...
	private EntityManager() {
		...
		/*
		 * try { m_socket.setSoTimeout(200); } catch (SocketException e) { //
...
//	DelayedSend s = new DelayedSend();
	public void allConnected() throws IOException {
		...
		while (id < 4) {
			...
			if (m_connection.size() == 0) {
				m_connection.add(new connection(packet.getAddress(), packet
						.getPort()));
			} else {
				for (int i = 0; i < m_connection.size(); i++) {
					if (!(m_connection.get(i).m_address == packet.getAddress() && m_connection
							.get(i).m_port == packet.getPort())) {
						m_connection.add(new connection(packet.getAddress(),
								packet.getPort()));
						break;
					}
				}
			...
			if (msg - 1 < MAXPLAYERS) {
				for (int i = id; i < msg; i++) {
					m_entities.get(i).connect(packet.getAddress(),
							packet.getPort());
					byte[] out = new byte[4];
					byte[] in = new byte[128];
					out = ByteBuffer.allocate(4).putInt(id + 1).array();
					DatagramPacket send = new DatagramPacket(out, out.length,
					...
					while (msg1 != 0 && failTimer < 5) {
						...
						msg1 = ByteBuffer.wrap(get.getData(), 0,
								packet.getLength()).getInt();
						if (msg1 == 0 && id < 4)
							id++;
						else
							failTimer++;
	...
	public void updateinput() {
		...
		if (id >= 1 && id <= 4) {
			if (m_entities.get(id - 1).isUsedByPlayer()
					&& m_entities.get(id - 1).getAddress().getHostAddress()
	...
	public void broadcastPosition() {
		...
		MsgData msg = new MsgData(m_entities.get(0).getPosition(), m_entities.get(1)
				.getPosition(), m_entities.get(2).getPosition(), m_entities
				.get(3).getPosition(), m_entities.get(0).getDirection(),
				m_entities.get(1).getDirection(), m_entities.get(2)
						.getDirection(), m_entities.get(3).getDirection(),
				m_entities.get(4).getPosition(), ScoreKeeper.getInstance()
						.getScoreAsVector());
		...
		try {
			...
			for (int i = 0; i < m_connection.size(); i++) {
				DatagramPacket pack = new DatagramPacket(buf, buf.length,
						m_connection.get(i).m_address,
						m_connection.get(i).m_port);
